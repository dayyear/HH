...
namespace DRP
{
    ...
    public class CJ_TD
    {
        public static Resp Select(string ID = null, int rows = 10, int page = 1)
        {
            ...
            try
            {
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    {
                        cmd.CommandText = string.Format(@"SELECT COUNT(*) FROM [DRP].[CJ_TD] WHERE 1=1 {0}", where);
                        if (!string.IsNullOrWhiteSpace(ID))
                    ...
                    using (DbCommand cmd = cn.CreateCommand())
                    {
                        cmd.CommandText = string.Format(
                            @"SELECT A.*, 
                        ISNULL((SELECT SUM(RS) FROM [DRP].[CJ_DD_RS] WHERE ID IN (SELECT ID FROM [DRP].[CJ_DD] WHERE TD_ID=A.ID AND ZT IN('1','2'))), 0) AS YDRS,
                        RS - ISNULL((SELECT SUM(RS) FROM [DRP].[CJ_DD_RS] WHERE ID IN (SELECT ID FROM [DRP].[CJ_DD] WHERE TD_ID=A.ID AND ZT IN('1','2'))), 0) AS SYRS,
                        ...
                        FROM ( SELECT * FROM ( SELECT *, ROW_NUMBER () OVER (ORDER BY CFRQ DESC) AS RowNumber FROM [DRP].[CJ_TD] WHERE 1=1 {0}) AS X
                        WHERE RowNumber > {1} AND RowNumber <= {2} ) AS A 
                        LEFT JOIN [DRP].[CJ_TD_BJ] AS B ON A.ID=B.ID
                        ...
                        using (DbDataReader dr = cmd.ExecuteReader())
                        {
                            while (dr.Read())
                            {
                                ...
                                if (!Convert.IsDBNull(dr["SN"]) && !itemBJ.Where(x => x["SN"].ToString() == dr["SN"].ToString()).Any())
                                {
                                    ...
                                    itemBJ.Add(bj);
                                    for (int ordinal = 17; ordinal < 21; ordinal++)
                                    {
                                        if (Convert.IsDBNull(dr[ordinal]))
        ...
        public static Resp Insert(IPrincipal User,
            string BH = null, string XLMC = null, string ZS = null, string CFRQ = null, string CLJZRQ = null,
            string ZT = null, string RS = null, string TS = null, string DFC = null, string XC = null, string QZCL = null,
            string[] BJMC = null, string[] MSJ = null, string[] FL = null)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(CLJZRQ)) throw new Exception("[材料截止日期]不能为空");
                try { DateTime.ParseExact(CLJZRQ, "yyyy-MM-dd", null); }
                catch (Exception) { throw new Exception(string.Format("[材料截止日期]格式不正确: {0}", CLJZRQ)); }
                ...
                if (string.IsNullOrWhiteSpace(QZCL)) throw new Exception("[签证材料]不能为空");
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    using (DbTransaction ta = cn.BeginTransaction())
                    {
                        using (DbCommand cmd = cn.CreateCommand())
                        {
                            ...
                            cmd.CommandText = "INSERT INTO [DRP].[CJ_TD](BH, XLMC, CFRQ, CLJZRQ, ZS, TS, DFC, RS, ZT, FBR, FBSJ) VALUES(@BH, @XLMC, @CFRQ, @CLJZRQ, @ZS, @TS, @DFC, @RS, @ZT, @FBR, @FBSJ);SELECT  @@IDENTITY";
                            ...
                            Common.AddParameter(cmd, "@CLJZRQ", DbType.Date).Value = DateTime.Parse(CLJZRQ);
                            ...
                            cmd.CommandText = "INSERT INTO [DRP].[CJ_TD_FJ](ID, XC, QZCL) VALUES(@ID, @XC, @QZCL)";
                            cmd.Parameters.Clear();
                            ...
                            Common.AddParameter(cmd, "@QZCL", DbType.Binary).Value = File.ReadAllBytes(HttpContext.Current.Server.MapPath("~/pub/attachment/" + QZCL));
                            cmd.ExecuteNonQuery();
                            ...
                            for (int i = 0; i < BJMC.Length; i++)
                            {
                                ...
                                cmd.CommandText = "INSERT INTO [DRP].[CJ_TD_BJ](ID,SN,BJMC,MSJ,FL) VALUES(@ID,@SN,@BJMC,@MSJ,@FL)";
                                cmd.Parameters.Clear();
                                Common.AddParameter(cmd, "@ID", DbType.String).Value = resp.message;
            ...
            try
            {
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    using (DbTransaction ta = cn.BeginTransaction())
                    {
                        ...
                        {
                            cmd.Transaction = ta;
                            cmd.CommandText = "INSERT INTO [DRP].[CJ_TD](BH, XLMC, CFRQ, CLJZRQ, ZS, TS, DFC, RS, ZT, FBR, FBSJ) SELECT BH, XLMC, CFRQ, CLJZRQ, ZS, TS, DFC, RS, ZT, @FBR, @FBSJ FROM [DRP].[CJ_TD] WHERE ID=@ID;SELECT  @@IDENTITY";
                        ...
                        {
                            cmd.Transaction = ta;
                            cmd.CommandText = "INSERT INTO [DRP].[CJ_TD_BJ](ID, SN, BJMC, MSJ, FL) SELECT @ID1, SN, BJMC, MSJ, FL FROM [DRP].[CJ_TD_BJ] WHERE ID=@ID2";
                        ...
                        {
                            cmd.Transaction = ta;
                            cmd.CommandText = "INSERT INTO [DRP].[CJ_TD_FJ](ID, XC, QZCL) SELECT @ID1, XC, QZCL FROM [DRP].[CJ_TD_FJ] WHERE ID=@ID2";
                            Common.AddParameter(cmd, "@ID1", DbType.Int32).Value = resp.message;
                            Common.AddParameter(cmd, "@ID2", DbType.Int32).Value = ID;
                            cmd.ExecuteNonQuery();
                        }//DbCommand
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    using (DbTransaction ta = cn.BeginTransaction())
                    {
                        using (DbCommand cmd = cn.CreateCommand())
                        {
                            ...
                            cmd.CommandText = "UPDATE [DRP].[CJ_TD] SET BH=@BH, XLMC=@XLMC, CFRQ=@CFRQ, CLJZRQ=@CLJZRQ, ZS=@ZS, TS=@TS, DFC=@DFC, RS=@RS, ZT=@ZT WHERE ID=@ID";
                            cmd.Parameters.Clear();
                            ...
                            {
                                cmd.CommandText = "UPDATE [DRP].[CJ_TD_FJ] SET XC=@XC WHERE ID=@ID";
                                cmd.Parameters.Clear();
                                Common.AddParameter(cmd, "@ID", DbType.String).Value = ID.Trim();
                                Common.AddParameter(cmd, "@XC", DbType.Binary).Value = File.ReadAllBytes(HttpContext.Current.Server.MapPath("~/pub/attachment/" + XC));
                                cmd.ExecuteNonQuery();
                            }
                            if (!string.IsNullOrWhiteSpace(QZCL))
                            {
                                cmd.CommandText = "UPDATE [DRP].[CJ_TD_FJ] SET QZCL=@QZCL WHERE ID=@ID";
                                cmd.Parameters.Clear();
                                Common.AddParameter(cmd, "@ID", DbType.String).Value = ID.Trim();
                                Common.AddParameter(cmd, "@QZCL", DbType.Binary).Value = File.ReadAllBytes(HttpContext.Current.Server.MapPath("~/pub/attachment/" + XC));
                                cmd.ExecuteNonQuery();
                            }

                            cmd.CommandText = "DELETE FROM [DRP].[CJ_TD_BJ] WHERE ID=@ID";
                            cmd.Parameters.Clear();
                            Common.AddParameter(cmd, "@ID", DbType.String).Value = ID.Trim();
                            cmd.ExecuteNonQuery();
                            for (int i = 0; i < BJMC.Length; i++)
