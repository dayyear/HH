            ...
            int rows = 10, int page = 1)
        {
            ...
            try
            {
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    {
                        cmd.CommandText = string.Format(@"SELECT COUNT(*) FROM [DRP].[GN_DD] WHERE 1=1 {0}", where);
                        if (!string.IsNullOrWhiteSpace(ID))
                    ...
                    using (DbCommand cmd = cn.CreateCommand())
                    {
                        cmd.CommandText = string.Format(
                            ...
                            FROM ( SELECT * FROM ( SELECT *, ROW_NUMBER () OVER (ORDER BY YDSJ DESC) AS RowNumber FROM [DRP].[GN_DD] WHERE 1=1 {0}) AS X
                            WHERE RowNumber > {1} AND RowNumber <= {2}
                            ) AS A LEFT JOIN [DRP].[GN_DD_RS] AS B ON A.ID=B.ID
                            LEFT JOIN [DRP].[GN_DD_MD] AS C ON A.ID=C.ID
            ...
            string[] XM, string[] SFZH, string[] XB, string[] CSRQ, string[] LXDH, string[] SFJS)
        {
            ...
            try
            {
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    using (DbCommand cmd = cn.CreateCommand())
                    {
                        cmd.CommandText =
                            ...
                            FROM [DRP].[GN_TD] AS X WHERE ID=@ID";
                    ...
                    {
                        cmd.CommandText =
                            @"SELECT * FROM [DRP].[GN_TD_BJ] WHERE ID=@ID ORDER BY SN";
                        Common.AddParameter(cmd, "@ID", DbType.String).Value = TD_ID;
                        using (DbDataReader dr = cmd.ExecuteReader())
                        {
                            while (dr.Read())
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    using (DbCommand cmd = cn.CreateCommand())
                    {
                        cmd.CommandText = @"SELECT ISNULL(SUM(RS),0) FROM [DRP].[GN_DD_RS] WHERE ID IN (SELECT ID FROM [DRP].[GN_DD] WHERE TD_ID = @TD_ID AND ZT<>'0')";
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    using (DbTransaction ta = cn.BeginTransaction())
                    {
                        ...
                        using (DbCommand cmd = cn.CreateCommand())
                        {
                            ...
                            cmd.CommandText =
                                @"INSERT INTO 
                                [DRP].[GN_DD](TD_ID, TD_BH, TD_XLMC, TD_CFRQ, TD_ZS, TD_TS, TD_DFC, TD_RS, TD_FBR, TD_FBSJ, SCD, BZ, JBR, JBRSJ, ZT, YDR, YDSJ) 
                        ...
                        using (DbCommand cmd = cn.CreateCommand())
                        {
                            ...
                            for (int i = 0; i < RS.Length; i++)
                            {
                                ...
                                cmd.CommandText =
                                    @"INSERT INTO [DRP].[GN_DD_RS](ID,SN,BJMC,MSJ,FL,RS,RS_DFC,FYXJ) 
                            ...
                            cmd.CommandText = "UPDATE [DRP].[GN_DD] SET BH=@BH, FYHJ=@FYHJ, JSJG=@JSJG WHERE ID=@ID";
                            cmd.Parameters.Clear();
                            Common.AddParameter(cmd, "@ID", DbType.String).Value = ID;
                            Common.AddParameter(cmd, "@BH", DbType.String).Value = "GN" + int.Parse(ID).ToString("00000000");
                        ...
                        using (DbCommand cmd = cn.CreateCommand())
                        {
                            ...
                            for (int i = 0; i < XM.Length; i++)
                            {
                                ...
                                cmd.CommandText =
                                    @"INSERT INTO [DRP].[GN_DD_MD](ID,SN,XM,SFZH,XB,CSRQ,LXDH,SFJS) 
            ...
            try
            {
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    using (DbTransaction ta = cn.BeginTransaction())
                    {
                        ...
                        {
                            cmd.Transaction = ta;
                            cmd.CommandText = @"DELETE FROM [DRP].[GN_DD_MD] WHERE ID=@ID";
                            Common.AddParameter(cmd, "@ID", DbType.String).Value = ID;
                        ...
                        if (!string.IsNullOrWhiteSpace(ZT))
                            ...
                            {
                                cmd.Transaction = ta;
                                cmd.CommandText = "UPDATE [DRP].[GN_DD] SET ZT=@ZT WHERE ID=@ID";
                                Common.AddParameter(cmd, "@ID", DbType.String).Value = ID;
