...
namespace DRP
{
    ...
    public class CJ_DD
    {
            ...
            int rows = 10, int page = 1)
        {
            ...
            try
            {
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    {
                        cmd.CommandText = string.Format(@"SELECT COUNT(*) FROM [DRP].[CJ_DD] WHERE 1=1 {0}", where);
                        if (!string.IsNullOrWhiteSpace(ID))
                    ...
                    using (DbCommand cmd = cn.CreateCommand())
                    {
                        cmd.CommandText = string.Format(
                            @"SELECT A.*, 
                                ...
                                (SELECT MC FROM [DRP].[DM] WHERE LX='DD_ZT' AND DM=A.ZT) AS ZT1,
                                B.SN AS SN1,B.BJMC,B.MSJ,B.FL,B.RS,B.RS_DFC,B.FYXJ,
                                C.SN AS SN2,C.XM,C.SFZH,C.XB,C.CSRQ,C.LXDH,C.SFJS
                            FROM ( SELECT * FROM ( SELECT *, ROW_NUMBER () OVER (ORDER BY YDSJ DESC) AS RowNumber FROM [DRP].[CJ_DD] WHERE 1=1 {0}) AS X
                            WHERE RowNumber > {1} AND RowNumber <= {2}
                            ) AS A LEFT JOIN [DRP].[CJ_DD_RS] AS B ON A.ID=B.ID
                            LEFT JOIN [DRP].[CJ_DD_MD] AS C ON A.ID=C.ID
                            ORDER BY A.RowNumber,B.SN,C.SN", where, rows * (page - 1), rows * page);
                        if (!string.IsNullOrWhiteSpace(ID))
                            Common.AddParameter(cmd, "@ID", DbType.String).Value = ID;
                        if (!User.IsInRole("ADMIN"))
                            Common.AddParameter(cmd, "@YDR", DbType.String).Value = User.Identity.Name;
                        using (DbDataReader dr = cmd.ExecuteReader())
                        {
                            while (dr.Read())
                            {
                                ...
                                else
                                {
                                    ...
                                    resp.items.Add(item);
                                    for (int ordinal = 0; ordinal < 26; ordinal++)
                                    {
                                ...
                                if (!Convert.IsDBNull(dr["SN1"]) && itemRS.Where(x => x["SN"].ToString() == dr["SN1"].ToString()).Count() == 0)
                                {
                                    ...
                                    itemRS.Add(rs);
                                    for (int ordinal = 26; ordinal < 33; ordinal++)
                                    {
                                        if (Convert.IsDBNull(dr[ordinal]))
                                ...
                                if (!Convert.IsDBNull(dr["SN2"]) && itemMD.Where(x => x["SN"].ToString() == dr["SN2"].ToString()).Count() == 0)
                                {
                                    ...
                                    itemMD.Add(md);
                                    for (int ordinal = 33; ordinal < 40; ordinal++)
                                    {
                                        if (Convert.IsDBNull(dr[ordinal]))
        ...
        public static Resp Insert(
            IPrincipal User, string TD_ID,
            string SCD, string BZ, string JBR, string JBRSJ,
            ...
            string[] XM, string[] SFZH, string[] XB, string[] CSRQ, string[] LXDH, string[] SFJS)
        {
            ...
            try
            {
                ...
                //////////////////////////////////////////////////////////////////////////
                //团队基本信息
                string TD_BH, TD_XLMC, TD_CFRQ, TD_CLJZRQ, TD_ZS, TD_TS, TD_DFC, TD_RS, TD_FBR, TD_FBSJ;
                //团队报价信息
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    using (DbCommand cmd = cn.CreateCommand())
                    {
                        cmd.CommandText =
                            ...
                            FROM [DRP].[CJ_TD] AS X WHERE ID=@ID";
                    ...
                    {
                        cmd.CommandText =
                            @"SELECT * FROM [DRP].[CJ_TD_BJ] WHERE ID=@ID ORDER BY SN";
                        Common.AddParameter(cmd, "@ID", DbType.String).Value = TD_ID;
                        using (DbDataReader dr = cmd.ExecuteReader())
                        {
                            while (dr.Read())
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    using (DbCommand cmd = cn.CreateCommand())
                    {
                        cmd.CommandText = @"SELECT ISNULL(SUM(RS),0) FROM [DRP].[CJ_DD_RS] WHERE ID IN (SELECT ID FROM [DRP].[CJ_DD] WHERE TD_ID = @TD_ID AND ZT<>'0')";
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    using (DbTransaction ta = cn.BeginTransaction())
                    {
                        ...
                        using (DbCommand cmd = cn.CreateCommand())
                        {
                            ...
                            cmd.CommandText =
                                @"INSERT INTO 
                                [DRP].[CJ_DD](TD_ID, TD_BH, TD_XLMC, TD_CFRQ, TD_CLJZRQ, TD_ZS, TD_TS, TD_DFC, TD_RS, TD_FBR, TD_FBSJ, SCD, BZ, JBR, JBRSJ, ZT, YDR, YDSJ) 
                                VALUES(@TD_ID, @TD_BH, @TD_XLMC, @TD_CFRQ, @TD_CLJZRQ, @TD_ZS, @TD_TS, @TD_DFC, @TD_RS, @TD_FBR, @TD_FBSJ, @SCD, @BZ, @JBR, @JBRSJ, @ZT, @YDR, @YDSJ);
                                SELECT  @@IDENTITY";
                            ...
                            Common.AddParameter(cmd, "@TD_CLJZRQ", DbType.String).Value = string.IsNullOrWhiteSpace(TD_CLJZRQ) ? Convert.DBNull : TD_CFRQ.Trim();
                        ...
                        using (DbCommand cmd = cn.CreateCommand())
                        {
                            ...
                            for (int i = 0; i < RS.Length; i++)
                            {
                                ...
                                cmd.CommandText =
                                    @"INSERT INTO [DRP].[CJ_DD_RS](ID,SN,BJMC,MSJ,FL,RS,RS_DFC,FYXJ) 
                            ...
                            cmd.CommandText = "UPDATE [DRP].[CJ_DD] SET BH=@BH, FYHJ=@FYHJ, JSJG=@JSJG WHERE ID=@ID";
                            cmd.Parameters.Clear();
                            Common.AddParameter(cmd, "@ID", DbType.String).Value = ID;
                            Common.AddParameter(cmd, "@BH", DbType.String).Value = "CJ" + int.Parse(ID).ToString("00000000");
                        ...
                        using (DbCommand cmd = cn.CreateCommand())
                        {
                            ...
                            for (int i = 0; i < XM.Length; i++)
                            {
                                ...
                                cmd.CommandText =
                                    @"INSERT INTO [DRP].[CJ_DD_MD](ID,SN,XM,SFZH,XB,CSRQ,LXDH,SFJS) 
        ...
        public static Resp Update(
            IPrincipal User, string ID, string ZT,
            string[] XM, string[] SFZH, string[] XB, string[] CSRQ, string[] LXDH, string[] SFJS)
        {
            Resp resp = new Resp();
            try
            {
                ...
                using (DbConnection cn = df.CreateConnection())
                {
                    ...
                    using (DbTransaction ta = cn.BeginTransaction())
                    {
                        ...
                        {
                            cmd.Transaction = ta;
                            cmd.CommandText = @"DELETE FROM [DRP].[CJ_DD_MD] WHERE ID=@ID";
                            Common.AddParameter(cmd, "@ID", DbType.String).Value = ID;
                        ...
                        if (!string.IsNullOrWhiteSpace(ZT))
                            ...
                            {
                                cmd.Transaction = ta;
                                cmd.CommandText = "UPDATE [DRP].[CJ_DD] SET ZT=@ZT WHERE ID=@ID";
                                Common.AddParameter(cmd, "@ID", DbType.String).Value = ID;
