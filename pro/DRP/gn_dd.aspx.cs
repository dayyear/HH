...
public partial class pro_DRP_gn_dd : System.Web.UI.Page
{
    ...
    private void GET()
    {
        try
        {
            ...
            // 数据查询
            //////////////////////////////////////////////////////////////////////////
            Resp resp = DRP.GN_DD.Select(User, null, rows, page);
            ...
            else
            {
                ...
                foreach (var item in resp.items)
                {
                    ...
                    sb.AppendFormat("<td><a href='gn_dd1.aspx?VERB=UPDATE&ID={0}' target='_blank'>{1}</a></td>", item["ID"], item["BH"]);
                    sb.AppendFormat("<td><a href='gn_td1.aspx?VERB=SELECT&ID={0}' target='_blank'>{1}</a></td>", item["TD_ID"], HttpUtility.HtmlEncode(item["TD_XLMC"]));
                ...
                sb.AppendFormat("</td>");
                sb.AppendFormat("<td>");
                sb.AppendFormat("<a class='{0}' href='gn_dd.aspx?rows={1}&page={2}'>首页</a>", page <= 1 ? "btnDisable" : "btn", rows, 1);
                sb.AppendFormat("<a class='{0}' href='gn_dd.aspx?rows={1}&page={2}'>上一页</a>", page <= 1 ? "btnDisable" : "btn", rows, page - 1);
                sb.AppendFormat("<a class='{0}' href='gn_dd.aspx?rows={1}&page={2}'>下一页</a>", page * rows >= resp.total ? "btnDisable" : "btn", rows, page + 1);
                sb.AppendFormat("<a class='{0}' href='gn_dd.aspx?rows={1}&page={2}'>尾页</a>", page * rows >= resp.total ? "btnDisable" : "btn", rows, Math.Ceiling((double)resp.total / (double)rows));
                sb.AppendFormat("</td>");
                sb.AppendFormat("<td>");
