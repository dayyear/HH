...
public partial class pro_DRP_cj_td_fj : System.Web.UI.Page
{
    ...
    private void GET()
    {
        ...
        try
        {
            ...
            if (!Regex.IsMatch(TYPE, @"\A(XC|QZCL)\z", RegexOptions.IgnoreCase))
            ...
            else if (TYPE.ToUpper() == "QZCL")
                Response.AddHeader("content-disposition", string.Format("filename={0}.doc", Server.UrlPathEncode("签证材料")));
            ...
            using (DbConnection cn = df.CreateConnection())
            {
                ...
                {
                    cmd.CommandText = string.Format("SELECT {0} FROM [DRP].[CJ_TD_FJ] WHERE ID=@ID", TYPE);
