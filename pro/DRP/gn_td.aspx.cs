...
public partial class pro_DRP_gn_td : System.Web.UI.Page
{
    ...
    private void GET()
    {
        try
        {
            ...
            if (User.IsInRole("ADMIN"))
                toolbar.InnerHtml = "<a href='gn_td1.aspx?VERB=INSERT' class='btn' target='_blank'>发布</a>";
            ...
            // 数据查询
            //////////////////////////////////////////////////////////////////////////
            Resp resp = DRP.GN_TD.Select(null, rows, page);
            ...
            else
            {
                ...
                foreach (var item in resp.items)
                {
                    ...
                    sb.AppendFormat("<td><a href='gn_td1.aspx?VERB=SELECT&ID={0}' target='_blank'>{1}</a></td>", item["ID"], HttpUtility.HtmlEncode(item["BH"]));
                    ...
                    sb.AppendFormat("<td><a href='gn_td_fj.aspx?TYPE=XC&ID={0}' target='_blank'><img src='../../pub/img/word.gif'></a></td>", item["ID"]);
                ...
                sb.AppendFormat("</td>");
                sb.AppendFormat("<td>");
                sb.AppendFormat("<a class='{0}' href='gn_td.aspx?rows={1}&page={2}'>首页</a>", page <= 1 ? "btnDisable" : "btn", rows, 1);
                sb.AppendFormat("<a class='{0}' href='gn_td.aspx?rows={1}&page={2}'>上一页</a>", page <= 1 ? "btnDisable" : "btn", rows, page - 1);
                sb.AppendFormat("<a class='{0}' href='gn_td.aspx?rows={1}&page={2}'>下一页</a>", page * rows >= resp.total ? "btnDisable" : "btn", rows, page + 1);
                sb.AppendFormat("<a class='{0}' href='gn_td.aspx?rows={1}&page={2}'>尾页</a>", page * rows >= resp.total ? "btnDisable" : "btn", rows, Math.Ceiling((double)resp.total / (double)rows));
                sb.AppendFormat("</td>");
                sb.AppendFormat("<td>");
