$(document).ready(function () {
    ...
    $("form.condition").submit(function (e) {
        e.preventDefault();
        $("table.dataTable")
            .data("condition", $(this).find(":input").filter(function () { return this.value.length > 0 }).serialize())
            .data("page", 1)
    ...
    $("table.dataTable").on("refresh", function () {
        ...
        $(".loading").show();
        var start = new Date();
        $.post("dmJson.ashx?verb=select&rows=" + rows + "&page=" + page, condition, function (resp) {
            $(".loading").hide();
            if (!resp.success) {
                alert(resp.message);
                return;
    ...
    $("table.dataTable").on("click", "a.edit", function (e) {
        ...
        $dialog.dialog({
            ...
            buttons: {
                "保存": function () {
                    $(".loading").show();
                    $.post("dmJson.ashx?verb=update", $dialog.find("form").serialize(), function (resp) {
                        $(".loading").hide();
                        if (!resp.success) {
                            alert(resp.message);
                            return;
                ...
                "删除": function () {
                    if (!confirm("你确定要删除吗？删除之后将无法恢复！"))
                        return;
                    $(".loading").show();
                    $.post("dmJson.ashx?verb=delete", $dialog.find("form").serialize(), function (resp) {
                        $(".loading").hide();
                        if (!resp.success) {
                            alert(resp.message);
                            return;
    ...
    $("input.new").click(function (e) {
        ...
        $dialog.dialog({
            ...
            buttons: {
                "保存": function () {
                    $(".loading").show();
                    $.post("dmJson.ashx?verb=insert", $dialog.find("form").serialize(), function (resp) {
                        $(".loading").hide();
                        if (!resp.success) {
                            alert(resp.message);
                            return;
